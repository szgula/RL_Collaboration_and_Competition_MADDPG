...
class myMADDPG(Agent):
    def __init__(self, state_size, action_size, number_of_independednt_agents, random_seed, enable_priority_2=False,
        ...
        super().__init__(state_size, action_size, number_of_independednt_agents, random_seed,
                         enable_priority_2, BUFFER_SIZE, BATCH_SIZE,
                         GAMMA, TAU, LR_ACTOR, LR_CRITIC, WEIGHT_DECAY, index_of_agent_in_maddpg)

    def set_other_agents(self, actors):
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        #error = self.get_priority(s, a, r, ns, d)      #TODO : implement support for multiagent
        error = 1
        #flatten all
    ...
    def learn(self, experiences, gamma):
        ...
        nb_of_agents = self.number_of_agents
        actions_agent_split = [actions[:, i*action_size:(i+1)*action_size] for i in range(nb_of_agents)]
        states_agent_split = [states[:, i*state_size:(i+1)*state_size] for i in range(nb_of_agents)]
        next_states_agent_split = [next_states[:, i*state_size:(i+1)*state_size] for i in range(nb_of_agents)]

        actions_next = [actor.actor_target(next_states_agent_split[idx]).detach()
                        for idx, actor in enumerate(self.all_actors)]

        actions_next_flatten = torch.cat((actions_next[0], actions_next[1]), 1)     #TODO add support for other number of agents - not just 2
        with torch.no_grad():
            Q_targets_next = self.critic_target(next_states, actions_next_flatten)
        # Compute Q targets for current states (y_i) TODO: check if shuldn't be any(dones)
        Q_targets = rewards[:, self.agent_index].view(self.BATCH_SIZE, 1) + (gamma * Q_targets_next * (1 - dones[:, self.agent_index].view(self.BATCH_SIZE, 1)))
        # Compute critic loss
        ...
        # ---------------------------- update actor ---------------------------- #
        # Compute actor loss
        actions_pred = [actor.actor_local(states_agent_split[idx]) if idx == self.agent_index
                        else actor.actor_local(states_agent_split[idx]).detach()
                        for idx, actor in enumerate(self.all_actors)]
        actions_pred_flatten = torch.cat((actions_pred[0], actions_pred[1]), 1)  # TODO add support for other number of agents - not just 2
        actor_loss = -self.critic_local(states, actions_pred_flatten).mean()
        # Minimize the loss
